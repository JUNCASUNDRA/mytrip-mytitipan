...
Scenario: Traveler views request list dashboard
  ...
  Then I should see my requests grouped by active status categories:
    ...
    | Active | Requests in progress in status "In Progress" or "Ready for Delivery" |
    | Completed | Finalized requests in status "Completed" |
```
...
Scenario: Traveler views request timeline log
  ...
  Then I should see the status history logs showing:
    ...
    | In Progress | Traveler updated status to In Progress |
    | Ready for Delivery | Traveler marked ready for delivery |
    | Completed | Delivery or handover confirmed |
```
